...
into test cases.
Out-of-the-box a set of built-in test modules are registered to be used with the
harness, but you can also register your own custom test modules.
...
Human language rendering is only supported for message tags emitted from
built-in test modules.
(If you need human language rendering of message tags emitted from custom test
modules, please let us know.)

For the tooling to find the message tags the source code needs to be structured
in a certain way.
...
Since human language rendering is only supported for built-in test modules,
custom test modules should just return an empty hashref.
